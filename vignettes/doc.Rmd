...
CHETAHClassifier = function(expr, clusters, target){
    ...
    ratio <- as.numeric(sort(table(final_predictions), decreasing = T)[1]) /
...
res_max_change = advMaxChange(rna_matrix, clusters_id, "DC", CHETAHClassifier,adv_method="perc99", max_split_size = 100, verbose=T)
res_max_change
```
...
min_change_overview <- minChangeOverview(mat_pbmc, cell_types, CHETAHClassifier, modifications= modifications)
min_change_overview
```
...
max_change_overview <- maxChangeOverview(mat_pbmc, cell_types, CHETAHClassifier, modifications= modifications)
max_change_overview
```
