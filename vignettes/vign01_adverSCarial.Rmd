...
Apply the Inferred Gradient Descent for Cluster (IGD4C) attack on the "Memory CD4 T" cells cluster with parameters alpha=epsilon=1

```{r IGD4C1, warning = FALSE}
resIGD4C <- advIGD4C(as.data.frame(matPbmc), cellTypes,
...
```{r IGD4C2, warning = FALSE}
tail(resIGD4C$byGeneSummary)
```
```{r IGD4C2_bis, warning = FALSE}
resIGD4C$oneRowSummary
```
You can retrieve the modified matrix.

```{r IGD4C3, warning = FALSE}
modifiedMatrix <- resIGD4C$expr
```

And visualize the list of modified genes.

```{r IGD4C4, warning = FALSE}
resIGD4C$modGenes
```

Check the new classification of the modified matrix.

```{r IGD4C5, warning = FALSE}
