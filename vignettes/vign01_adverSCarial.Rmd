...
lfile <- connect(filename = pbmcPath, mode = "r+", skip.validate = TRUE)
matPbmc <- as.data.frame(lfile[['matrix']][,][[1]])
```
...
cellTypes <- lfile[["row_attrs/cell_type"]][]
```
...
genesMinChange = advMinChange(matPbmc, cellTypes, "DC",
...
The function found that modifying the single gene `names(genesMinChange@values)[1]` with the `perc99` modification on the cluster leads to a new classification, `genesMinChange@values[[1]][1]`.
...
genesMaxChange <- advMaxChange(matPbmc, cellTypes, "DC", MClassifier,
...
The function found `length(genesMaxChange@values)` genes that you can modify with the `perc99` modification, and the cluster is still classified as `DC`.
...
matMaxAdver <- advModifications(matPbmc, genesMaxChange@values, cellTypes, "DC")
```
